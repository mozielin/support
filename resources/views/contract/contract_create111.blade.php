...
<div class="row">
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            <!--先查詢公司統編取得ID及資訊-->
            <form class="form-horizontal" method="post" action="{{route('contract_get')}}">
                {{ csrf_field() }}
                <!--if($num=='1')
                <div class="form-group{{ $errors->has('company_EIN') ? ' has-error' : '' }}">
                    <label for="company" class="col-md-4 control-label">公司名稱</label>
                    <div class="col-md-6">
                ...
                <div class="form-group{{ $errors->has('company_EIN') ? ' has-error' : '' }}">
                    <label for="company_EIN" class="col-md-4 control-label">公司名稱</label>
                    <div class="col-md-6">
                        <input id="company_EIN" type="text" class="form-control" name="company_EIN" placeholder="請輸入公司名稱查詢" value="{{ old('company_EIN') }}" required autofocus>
                        if ($errors->has('company_EIN'))
                        <span class="help-block">
                            <strong>{{ $errors->first('company_EIN') }}</strong>
                        </span>
                        endif
                    </div>
            ...
            <form class="form-horizontal" method="post" action="/contract/upload" enctype="multipart/form-data">
                {{ csrf_field() }}
                <div class="form-group{{ $errors->has('autosearch') ? ' has-error' : '' }}">
                    <label for="autosearch" class="col-md-4 control-label">公司名稱</label>
                    <div class="col-md-6">
                        <input id="autosearch" type="text" class="form-control" name="autosearch" placeholder="請輸入公司名稱查詢" value="" required autofocus>
                        @if ($errors->has('autosearch'))
                        <span class="help-block">
                            <strong>{{ $errors->first('autosearch') }}</strong>
                        </span>
                        @endif
                    </div>
                </div>
